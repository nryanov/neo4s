...
object Read extends ReadLowerPriority {
  ...
  implicit def recordRead[R <: Symbol, H, T <: HList](implicit
    H: Lazy[Read[H]],
    T: Lazy[Read[T]]
...
trait ReadLowestPriority {
  ...
  implicit def ohcons1[H, T <: HList](implicit
    ...
    new Read[Option[H :: T]]((record, idx) =>
      for {
  ...
  implicit def ohcons2[H, T <: HList](implicit
    ...
    new Read[Option[Option[H] :: T]]((record, idx) => T.value.unsafeGet(record, idx + 1).map(H.value.unsafeGet(record, idx) :: _))
